```tsx
#I want to be able to edit the timer details, such as name, category and time when i go on the edit-timer screen
...
export default function HomeScreen() {
  ...
  const loadTimers = async () => {
    ...
    if (savedTimers) {
      ...
      setCategories([...new Set(parsedTimers.map((t) => t.category))]);
    } else {
      setTimers([]);
      setCategories([]);
    }
...
export default function EditTimerScreen() {
  ...
  useEffect(() => {
    if (timerString) {
      setTimer(JSON.parse(timerString));
    }
  }, [timerString]);
  ...
  const handleSave = async () => {
    if (timer) {
      ...
      const updatedTimers = timers.map((t: Timer) => 
        t.name === timer.name && t.category === timer.category ? timer : t
      );
      await AsyncStorage.setItem('timers', JSON.stringify(updatedTimers));
      router.back();
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        onChangeText={(text) => setTimer({ ...timer, duration: parseInt(text) || 0 })}
